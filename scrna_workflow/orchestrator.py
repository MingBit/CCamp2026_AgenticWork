...
# Input files hashed into the run fingerprint.
INPUT_KEYS = ('input_path','metadata_path','tf_list','motif_evidence_path','marker_path','knowledge_base_path',
...
def selected_tasks(targets):
    """Return the dependency closure in pipeline order for requested stages."""
    targets = targets or ['report']
    unknown = set(targets) - set(DEPS)
    if unknown:
        raise ValueError(f'Unknown workflow task(s): {", ".join(sorted(unknown))}')
    selected = set()
    def include(name):
    ...
    for target in targets:
        include(target)
    return {name: DEPS[name] for name in DEPS if name in selected}
...
def execute(config, resume=False):
    task_deps = selected_tasks(config.get('requested_tasks'))
    ...
    try:
        return _execute(config, root, resume, TASK_AGENTS, TASK_TOOLS, task_deps)
    finally:
        lock.unlink(missing_ok=True)

def _execute(config, root, resume, task_agents, task_tools, task_deps):
    versions = {}
    ...
    state = {'schema_version':1, 'fingerprint':fingerprint,'config':config,'inputs':inputs,
             ...
             'source_sha256':source_hash,'seed':config.get('seed',0),'dependencies':task_deps,
    ...
    functions = {name: task_agents[name] for name in task_deps}
    artifacts = {}
    pending = set(task_deps)
    changed = set()
    ...
    def call(name):
        ...
        for attempt in range(min(max(int(config.get('transient_retries',1)),0),2)+1):
            try:
                result = functions[name].execute(name, ctx)
                ...
                result = {'status':'failed','input_references':task_deps[name],'outputs':{},'metrics':{},
                ...
                return result
    while pending:
        ready = sorted(n for n in pending if all(d in artifacts for d in task_deps[n]))
        if not ready: raise RuntimeError('Dependency cycle')
        runnable = []
        for name in ready:
            previous = state['tasks'].get(name)
            if previous and not any(d in changed for d in task_deps[name]) and valid_checkpoint(previous):
            ...
            blocked = [d for d in task_deps[name] if artifacts[d]['status'] in ('failed','blocked')]
            ...
            else: changed.add(name); runnable.append(name); event({'task':name,'event':'call','agent':functions[name].name,'tool':task_tools[name].__name__,'dependencies':task_deps[name]})
